...
int main() {
  ...
  while (getline(in_map_, line)) {
    ...
    // TODO : Start in Lane 1 which is middle lane
    int lane = 1;
    
    // TODO : Reference velocity to target
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for ( int i = 0; i < sensor_fusion.size(); i++ ) {
                ...
                // see if any vehicle is in ahead of my car or in the left or right lanes within vicinity
            ...
            if (car_ahead)
            {
                ...
                vector<double> lane_costs ={0.0,0.0,0.0};;
                if (lane == 0){
                    if (car_right)
                        lane_costs[lane+1] += pow(10,5);
                    else
                        lane_costs[lane] += pow(10,5);
                    std::cout << "In lane zero....is there a car on right?  " << car_right << std::endl;
                    }else if (lane == 1){
                    if (car_left)
                        lane_costs[lane-1] += pow(10,5);
                    if (car_right)
                    ...
                    if (!(car_left || car_right))
                        lane_costs[lane] += pow(10,5);
                    std::cout << "In lane 1....is there a car on left  " << car_left << " or right " << car_right << std::endl;
                }else{
                    if (car_left)
                        lane_costs[lane-1] += pow(10,5);
                    else
                        lane_costs[lane] += pow(10,5);
                    std::cout << "In lane two....is there a car on left?  " << car_left << std::endl;
                }
                
                for (int i = 0; i < lane_costs.size(); i++)
                {
                    double cost = lane_costs[i];
            ...
            // If prev size is empty use the car as ref starting point
            if ( prev_size < 2 ) {
                ...
                // Use previous paths end point as the starting reference
            ...
            // Define actual points we will use for planner
